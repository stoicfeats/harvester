...
export const TweetCard: React.FC<{ tweet: Tweet; darkMode: boolean; onToggleStar: (tweetId: string) => void }> = ({ tweet, darkMode, onToggleStar }) => {
    ...
    >
      ...
      <button
        onClick={(e) => {
          e.preventDefault();
          e.stopPropagation();
          onToggleStar(tweet.id);
        }}
        className={`absolute top-4 right-4 z-20 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${tweet.isStarred ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
        style={{
          backgroundColor: theme.BG,
          boxShadow: theme.SHADOW,
          color: tweet.isStarred ? '#ff4d00' : 'inherit'
        }}
      >
        <svg
          width="16"
          height="16"
          viewBox="0 0 24 24"
          fill={tweet.isStarred ? "currentColor" : "none"}
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
